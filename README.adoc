...
* `docker-compose -f docker-compose.psql.test.yml up` -- Isolated PostgreSQL test stack (xref:./docker-compose.psql.test.yml[config])
...
=== Isolated Test Server With Cloned PostgreSQL State

If you need a second server with the same character/state data while your main server stays online, use the test stack and clone helper:

[source,bash]
----
./deploy/postgresql/start_test_server_with_db_clone.sh
----

This starts a separate `ddon-db-test` + `ddon-server-test` pair and clones data from the running main DB container (`ddon-db`) into the test DB.
The test stack uses `GameServerList.test.csv` so login hands off clients to test game port `53000` instead of main.

Port mapping for the test stack:
...
If your client runs on another machine, update `IpAddress` in `Arrowgene.Ddon.Shared/Files/Assets/GameServerList.test.csv` from `127.0.0.1` to the host/LAN IP of this server.

Stop test stack:

[source,bash]
----
docker compose -p ddon-test -f docker-compose.psql.test.yml down
----
